    ...
    <Navbar />  
      ...
      <div class="tests-container"> 
        ...
        <table class="tests-table">
          ...
          <tbody>
            <tr v-for="result in results" :key="result.id">
              ...
              <td><button class="btn btn-outline color-btn" @click="viewTestDetails(result.id)">查看报告</button></td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="pagination">
          <button class="btn btn-outline color-btn" @click="prevPage" :disabled="currentPage === 1">上一页</button>
          <span class="page-info">第 {{ currentPage }} 页，共 {{ totalPages }} 页</span>
          <button class="btn btn-outline color-btn " @click="nextPage" :disabled="currentPage === totalPages">下一页</button>
        </div>
      </div>
